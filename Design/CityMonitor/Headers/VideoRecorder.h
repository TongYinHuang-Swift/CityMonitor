...
#ifdef WIN32
#define RECORD_DATA_PATH	    "c:\\data\\"
#else
#define RECORD_DATA_PATH	    "./"
#endif

#define MAX_FILE_LEN			(102400000)//(10240/5)
...
public:
	...
	FILE*			CreateFile(char* path);							// 创建该路径下的文件

	bool			CreateHistoryVideo();							// 创建新的索引文件并添加到历史视频列表中   
	bool            RefreshCurHistoryVideo();						// 视频流文件读取保存完毕，更新该视频流记录信息  
	bool            RefreshCurHistoryVideo(uint videoLength);						// 视频流文件读取保存完毕，更新该视频流记录信息,传入视频长度
